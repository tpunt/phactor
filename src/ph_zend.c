...
#include "src/ph_zend.h"

/*
The following is taken from Zend/zend_hash.c and changes the copying of array
values to use PH_ZVAL_DUP rather than ZVAL_COPY_VALUE (as well as performing a
...
static zend_always_inline int ph_zend_array_dup_element(HashTable *source, HashTable *target, uint32_t idx, Bucket *p, Bucket *q, int packed, int static_keys, int with_holes)
{
	zval *data = &p->val;
...
/* Newly commented lines - we don't increment the refcount since we perform a hard copy instead
	...
	PH_ZVAL_DUP(&q->val, data); // Changed line here (from ZVAL_COPY_VALUE)
...
static zend_always_inline void ph_zend_array_dup_packed_elements(HashTable *source, HashTable *target, int with_holes)
{
	...
	do {
		if (!ph_zend_array_dup_element(source, target, 0, p, q, 1, 1, with_holes)) {
			if (with_holes) {
				ZVAL_UNDEF(&q->val);
			}
		}
...
static zend_always_inline uint32_t ph_zend_array_dup_elements(HashTable *source, HashTable *target, int static_keys, int with_holes)
{
	uint32_t idx = 0;
	Bucket *p = source->arData;
	Bucket *q = target->arData;
	...
	do {
		if (!ph_zend_array_dup_element(source, target, idx, p, q, 0, static_keys, with_holes)) {
			uint32_t target_idx = idx;

			idx++; p++;
			while (p != end) {
				if (ph_zend_array_dup_element(source, target, target_idx, p, q, 0, static_keys, with_holes)) {
...
HashTable* ZEND_FASTCALL ph_zend_array_dup(HashTable *source)
{
	uint32_t idx;
	HashTable *target;
	...
	if (source->nNumUsed == 0) {
		...
		if (HT_IS_WITHOUT_HOLES(target)) {
			ph_zend_array_dup_packed_elements(source, target, 0);
		} else {
			ph_zend_array_dup_packed_elements(source, target, 1);
		}
		if (target->nNumOfElements > 0 &&
		    target->nInternalPointer == HT_INVALID_IDX) {
			idx = 0;
		...
		if (HT_HAS_STATIC_KEYS_ONLY(target)) {
			if (HT_IS_WITHOUT_HOLES(source)) {
				idx = ph_zend_array_dup_elements(source, target, 1, 0);
			} else {
				idx = ph_zend_array_dup_elements(source, target, 1, 1);
			}
		} else {
			if (HT_IS_WITHOUT_HOLES(source)) {
				idx = ph_zend_array_dup_elements(source, target, 0, 0);
			} else {
				idx = ph_zend_array_dup_elements(source, target, 0, 1);
			}
		}
		target->nNumUsed = idx;
...
zend_ast *ph_zend_ast_copy(zend_ast *ast)
{
	if (ast == NULL) {
		return NULL;
		...
		new->attr = ast->attr;
		PH_ZVAL_DUP(&new->val, zend_ast_get_zval(ast)); // Changed line (from ZVAL_COPY)
		return (zend_ast *) new;
...
constant AST types are copied. This internal function is used for PH_ZVAL_DUP
(defined in ph_zend.h), which is a thread-safe version of ZVAL_DUP.
*/
void ZEND_FASTCALL _ph_zval_copy_ctor_func(zval *zvalue ZEND_FILE_LINE_DC)
{
	if (EXPECTED(Z_TYPE_P(zvalue) == IS_ARRAY)) {
		ZVAL_ARR(zvalue, ph_zend_array_dup(Z_ARRVAL_P(zvalue))); // changed line
		...
		zend_ast *copy = ph_zend_ast_copy(Z_ASTVAL_P(zvalue)); // changed line
